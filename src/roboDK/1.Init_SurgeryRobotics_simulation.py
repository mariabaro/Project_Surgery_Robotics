...
def move_robot(robot, gripper, needle, text_label):
    ...
    # Track the endowrist orientation to compensate gripper rotation
    last_endo_roll = 0
    last_endo_pitch = 0
    last_endo_yaw = 0
    ...
    while True:
        ...
        if current_Endowrist_rpy:
            ...
            if robot.MoveL_Test(robot.Joints(), endowrist_pose_new) == 0:
                ...
                # Update tracked endowrist orientation
                last_endo_roll = endo_roll
                last_endo_pitch = endo_pitch
                last_endo_yaw = endo_yaw
            else:
                endowrist_orientation_msg = f"R={round(endo_roll)} P={round(endo_pitch)} W={round((endo_yaw+ZERO_YAW_TOOL)%360)}"
        ...
        if current_Gripper_rpy:
            ...
            # Move Gripper - compensate for endowrist rotation to keep gripper orientation independent
            gripper_pose = gripper.Pose()
            Xg, Yg, Zg, rg, pg, yg = Pose_2_TxyzRxyz(gripper_pose)
            
            # Apply gripper's own orientation, then compensate for endowrist rotation
            # The gripper pose is relative to endowrist, so we need to apply inverse of endowrist rotation
            gripper_pose_new = (transl(Xg, Yg, Zg) * 
                               rotz(math.radians(ZERO_YAW_GRIPPER)) * 
                               rotz(math.radians(g_yaw)) * 
                               roty(math.radians(g_pitch)) * 
                               rotx(math.radians(g_roll)) *
                               # Inverse endowrist rotation to compensate
                               rotx(math.radians(-last_endo_roll)) * 
                               roty(math.radians(-last_endo_pitch)) * 
                               rotz(math.radians(-last_endo_yaw)))
